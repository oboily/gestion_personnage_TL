            ...
            try:
                if sum_section > 1:
                    self._error = "L.%s S.%s: Cannot contain more than 1 section at time. " \
                                  "H1: %s, H2: %s, H3: %s, H4: %s, H5: %s." % (
                                      line_number, doc_sheet_name, is_first_section, is_second_section,
                                      is_third_section,
                                      is_fourth_section,
                                      is_fifth_section)
                    print(self._error, file=sys.stderr)
                ...
                if is_first_section:
                    ...
                    if not first_section:
                        self._error = "L.%s S.%s: Missing section H1 to insert section H3." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return

                    lst_section = first_section.get("section")
                    if not lst_section:
                        self._error = "L.%s S.%s: Missing section H2 to insert section H3." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return
                    ...
                    if not first_section:
                        self._error = "L.%s S.%s: Missing section H1 to insert section H4." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return

                    if not second_section:
                        self._error = "L.%s S.%s: Missing section H2 to insert section H4." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return
                    ...
                    lst_section = second_section.get("section")
                    if not lst_section:
                        self._error = "L.%s S.%s: Missing section H3 to insert section H4." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return
                    ...
                    if not first_section:
                        self._error = "L.%s S.%s: Missing section H1 to insert section H5." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return

                    if not second_section:
                        self._error = "L.%s S.%s: Missing section H2 to insert section H5." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return

                    if not third_section:
                        self._error = "L.%s S.%s: Missing section H3 to insert section H5." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return
                    ...
                    lst_section = third_section.get("section")
                    if not lst_section:
                        self._error = "L.%s S.%s: Missing section H4 to insert section H5." % (
                            line_number, doc_sheet_name)
                        print(self._error, file=sys.stderr)
                        return
                ...
                self._error = "L.%s S.%s: Unknown case of error: %s" % (line_number, doc_sheet_name, e)
                print(self._error, file=sys.stderr)
                return
        ...
        if second_bullet_description:
            if "description" not in section:
                self._error = "L.%s S.%s: Cannot create second-bullet description missing description " \
                              "for H%s." % (line_number, doc_sheet_name, header_lvl)
                print(self._error, file=sys.stderr)
                return False
            lst_description = section.get("description")
            ...
            if lst_description:
                ...
                self._error = "L.%s S.%s: Cannot create second-bullet description when not precede to bullet " \
                              "description for H%s." % (line_number, doc_sheet_name, header_lvl)
                print(self._error, file=sys.stderr)
                return False
